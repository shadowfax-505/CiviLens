...
RUN apt-get update \
    && apt-get install -y --no-install-recommends bash default-mysql-client libicu-dev libonig-dev libzip-dev poppler-utils tesseract-ocr tesseract-ocr-ben tesseract-ocr-eng unzip $PHPIZE_DEPS \
    && pecl install redis \
