<x-layouts.app :title="'Budget Detail - CivicLens'">
    ...
    <section class="mt-8 grid gap-6 lg:grid-cols-2">
        <div class="rounded-xl border bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
            ...
            @can('update', $budget)
                <form method="POST" action="{{ route('admin.finance.budgets.revisions.store', $budget) }}" class="mt-5 grid gap-3 rounded border p-3 dark:border-slate-800">
                    <p class="text-sm font-semibold">Record a revision</p>
                    @csrf
        ...
        <div class="rounded-xl border bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
            ...
            @can('update', $budget)
                <form method="POST" action="{{ route('admin.finance.budgets.transactions.store', $budget) }}" class="mt-5 grid gap-3 rounded border p-3 dark:border-slate-800">
                    <p class="text-sm font-semibold">Record a transaction</p>
                    @csrf
    ...
    <section class="mt-8 rounded-xl border bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
        <h2 class="text-xl font-semibold">Linked Procurement</h2>
        <p class="mt-1 text-sm text-slate-500">Tenders and contracts drawing on this budget.</p>
        <div class="mt-4 grid gap-6 lg:grid-cols-2">
            <ol class="space-y-2">
                @forelse ($budget->tenders as $tender)
                    <li class="rounded border px-3 py-2 text-sm dark:border-slate-800">
                        <a class="font-semibold text-blue-700 dark:text-blue-300" href="{{ route('admin.procurement.tenders.show', $tender) }}">{{ $tender->title }}</a>
                        <span class="text-slate-500">- {{ $tender->tender_number }}</span>
                    </li>
                @empty
                    <li class="text-sm text-slate-500">No tenders linked to this budget.</li>
                @endforelse
            </ol>
            <ol class="space-y-2">
                @forelse ($budget->contracts as $contract)
                    <li class="rounded border px-3 py-2 text-sm dark:border-slate-800">
                        <a class="font-semibold text-blue-700 dark:text-blue-300" href="{{ route('admin.procurement.contracts.show', $contract) }}">{{ $contract->contract_number }}</a>
                        <span class="text-slate-500">- {{ $contract->title }}</span>
                    </li>
                @empty
                    <li class="text-sm text-slate-500">No contracts linked to this budget.</li>
                @endforelse
            </ol>
        </div>
    </section>
