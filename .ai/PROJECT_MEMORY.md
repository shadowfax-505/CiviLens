...
- Sprint 13 part 1 production readiness and Civic Integrity Engine: production Docker/Nginx/PHP-FPM/Redis/MySQL scaffold, dedicated worker/scheduler roles, production environment example, public-safe health endpoint, scheduled `civiclens:integrity-run` command, reproducible `civic_intelligence_runs`, manual admin engine trigger, dashboard run status, repeat-winner and citizen-report-cluster rules, Pest tests, Playwright desktop/mobile/dark-mode/keyboard coverage, and documentation.
...
CivicLens v1 is finalized as `v1.0.0`; do not begin v2 AI infrastructure until production deployment is complete and ADR-backed.
