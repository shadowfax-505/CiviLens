...
- For RC1, `APP_VERSION` must be `v1.0.0-RC1`.
...
- Inside the production worker container, use `supervisorctl -c /etc/supervisord.conf status` to inspect queue and scheduler process state.
