...
- `poppler-utils` for native PDF extraction. `pdftotext` and `pdfinfo` are hard runtime requirements once extraction is enabled: the application image installs them, and `EXTRACTION_PDFTOTEXT_BINARY`/`EXTRACTION_PDFINFO_BINARY` should be absolute paths in production. Selective OCR additionally requires `tesseract-ocr` with the `ben` and `eng` language data; `EXTRACTION_TESSERACT_BINARY` and `EXTRACTION_PDFTOPPM_BINARY` should likewise be absolute paths. CI installs the same packages and asserts both `pdftotext -v` and the presence of the `ben` language before running the suite, so a missing toolchain fails loudly rather than silently skipping coverage.
