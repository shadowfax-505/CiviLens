...
Sprint 13 keeps production scaling simple: one PHP-FPM web service, supervised queue workers, a scheduler loop, MySQL, Redis, and Nginx. The Civic Integrity Engine is deterministic and can run from the scheduler while datasets are modest; at larger volumes, run it as a queue-backed or isolated command workload with the same rule services and run-history table.

Executive and integrity dashboards should remain aggregate-first. If record counts grow into the millions, add materialized aggregate tables or scheduled snapshots for dashboard cards, rule rankings, and report payloads while keeping operational source tables authoritative.
